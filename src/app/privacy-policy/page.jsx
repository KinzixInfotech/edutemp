import React from 'react';

export default function PrivacyPolicy() {
    return (
        <div className="max-w-4xl mt-25 mx-auto px-4 py-16 text-gray-800">
            <h1 className="text-3xl font-bold mb-2">Privacy Policy</h1>
            <p className="text-sm text-gray-500 mb-6">
                Last updated: February 24, 2026
            </p>

            <p className="mb-6">
                This Privacy Policy explains how <strong>EduBreezy</strong>, operated by{" "}
                <strong>Kinzix Infotech</strong>, collects, uses, and protects your
                information when you use our application and services.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                1. Interpretation and Definitions
            </h2>

            <ul className="list-disc pl-6 space-y-2 mb-6">
                <li>
                    <strong>Account:</strong> A unique account created to access the Service.
                </li>
                <li>
                    <strong>Application:</strong> EduBreezy mobile application.
                </li>
                <li>
                    <strong>Company:</strong> Kinzix Infotech, Katkamdag, Hazaribagh, Jharkhand, India.
                </li>
                <li>
                    <strong>Device:</strong> Any device such as a mobile phone or tablet.
                </li>
                <li>
                    <strong>Personal Data:</strong> Information that identifies an individual.
                </li>
            </ul>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                2. Information We Collect
            </h2>

            <h3 className="font-medium mt-4 mb-2">Personal Information</h3>
            <ul className="list-disc pl-6 space-y-2 mb-4">
                <li>Name</li>
                <li>Email address</li>
                <li>Phone number</li>
                <li>Contact address & Postal code</li>
                <li>Date of Birth, Gender, and Blood Group</li>
                <li>School-related details (class, section, role)</li>
            </ul>

            <h3 className="font-medium mt-4 mb-2">Usage Data</h3>
            <ul className="list-disc pl-6 space-y-2 mb-4">
                <li>Device type and OS</li>
                <li>IP address</li>
                <li>App activity and diagnostics</li>
            </ul>

            <h3 className="font-medium mt-4 mb-2">Location Data</h3>
            <ul className="list-disc pl-6 space-y-2 mb-6">
                <li>Precise location (if enabled)</li>
                <li>Background location (used only for features requiring continuous tracking, such as bus tracking)</li>
            </ul>

            {/* NEW SECTION: Location Information */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                3. Location Information
            </h2>
            <p className="mb-4">
                EduBreezy may collect and process location data, including precise location and background location, to provide features such as:
            </p>
            <ul className="list-disc pl-6 space-y-2 mb-4">
                <li>Live school bus tracking</li>
                <li>Student safety and monitoring</li>
                <li>Route and trip management</li>
            </ul>
            <p className="mb-4">
                Location data may be collected while the application is in use.
                Where required for specific features (for example, the Bus Driver role), location data may also be collected in the background.
            </p>
            <p className="mb-4">
                Background location access is used strictly for continuous tracking features necessary for transportation and safety services.
            </p>
            <p className="mb-4">
                Users may control or revoke location permissions at any time through their device settings.
            </p>

            <h2 className="text-xl font-semibold mt-10 mb-3">
                4. How We Use Your Information
            </h2>
            <ul className="list-disc pl-6 space-y-2 mb-6">
                <li>To provide and maintain the Service (including real-time bus tracking)</li>
                <li>To manage user accounts</li>
                <li>To support school administration (Attendance, Exams, Fees)</li>
                <li>To improve security and performance</li>
                <li>To comply with legal obligations</li>
            </ul>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                5. Data Sharing
            </h2>
            <p className="mb-4">
                We do not sell personal data, including location information. Data may be shared only with:
            </p>
            <ul className="list-disc pl-6 space-y-2 mb-6">
                <li>Authorized school authorities</li>
                <li>Trusted service providers</li>
                <li>Legal authorities when required by law</li>
            </ul>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                6. Children’s Privacy
            </h2>
            <p className="mb-6">
                EduBreezy is designed for schools and may process data of children under
                13 strictly for educational purposes. Data is collected only with school
                and parental authorization. We do not display ads or perform tracking
                for advertising.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                7. Data Retention
            </h2>
            <p className="mb-6">
                Personal data is retained only as long as required to provide services,
                comply with school policies, or meet legal obligations.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                8. Data Security
            </h2>
            <p className="mb-6">
                We implement reasonable technical and administrative safeguards to protect your personal data.
                However, no method of electronic storage or transmission is completely secure.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                9. Permissions & Controls
            </h2>
            <p className="mb-6">
                Users can manage or revoke permissions, including location access, at any time through their device settings. Disabling certain permissions may limit the functionality of some features.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                10. User Rights
            </h2>
            <ul className="list-disc pl-6 space-y-2 mb-6">
                <li>Access your personal data</li>
                <li>Request correction or deletion</li>
                <li>Contact us for privacy-related concerns</li>
            </ul>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                11. Changes to This Policy
            </h2>
            <p className="mb-6">
                We may update this Privacy Policy periodically. Updates will be posted
                on this page with a revised date.
            </p>

            {/* SECTION */}
            <h2 className="text-xl font-semibold mt-10 mb-3">
                12. Contact Us
            </h2>
            <p className="mb-2">If you have any questions, contact us:</p>
            <ul className="list-disc pl-6">
                <li>Company: Kinzix Infotech</li>
                <li>Phone: +91 94715 32682</li>
                <li>Location: Katkamdag, Hazaribagh, Jharkhand, India</li>
            </ul>
        </div>
    );
}
