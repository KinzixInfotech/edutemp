        ...
        const handleOAuthCallback = async () => {
            ...
            try {
                ...
                if (userData.role?.name !== 'SUPER_ADMIN') {
                    ...
                    toast.error('Access Denied', { description: 'This login is restricted to developers only' });
                    setGoogleLoading(false);
                    return;
                }
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            const { data, error: oauthError } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent('/developer-login?oauth=true')}`,
                },
            });
                            ...
                            <>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-5">
                                    ...
                                    <Button
                                        ...
                                        className="w-full h-12 rounded-xl font-medium border-gray-200 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 active:scale-[0.98]"
                                    >
                                        {googleLoading ? (
