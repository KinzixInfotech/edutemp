...
export default function NonTeachingStaffPage() {
    ...
    const { data: staffData = {}, isLoading, isFetching } = useQuery({
        queryKey: ['non-teaching-staff', schoolId, page, search, designationFilter, statusFilter, sortBy],
        queryFn: async () => {
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-1">
                    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold flex items-center gap-2">
                        <UserCog className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 flex-shrink-0" />
                    ...
                    <div className="border-t p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <p className="text-sm text-muted-foreground">
                                Showing {((page - 1) * itemsPerPage) + 1} to {Math.min(page * itemsPerPage, total)} of {total} staff members
                            </p>
                    ...
                    <DialogContent className="max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="flex items-start gap-4 pb-4 border-b">
                                <Avatar className="w-16 h-16">
                                    ...
                                    <AvatarFallback className="text-lg">
                                        {dialogData.name?.[0]?.toUpperCase() || dialogData.user?.name?.[0]?.toUpperCase() || "S"}
                                    </AvatarFallback>
                                </Avatar>
                        ...
                        <div className="space-y-6 mt-4">
                            ...
                            <div className="flex gap-3 pt-4 border-t">
                                <Link href={`/dashboard/schools/${schoolId}/profiles/staff/${dialogData.userId}`} className="flex-1">
