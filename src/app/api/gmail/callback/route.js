...
export async function GET(req) {
    ...
    try {
        ...
        const cookies = [
            `gmail_access_token=${tokens.access_token}; Path=/; Max-Age=3600; SameSite=Lax; HttpOnly; ${process.env.NODE_ENV === "production" ? "Secure;" : ""
            }`,
            tokens.refresh_token
                ? `gmail_refresh_token=${tokens.refresh_token}; Path=/; Max-Age=${60 * 60 * 24 * 30
                }; SameSite=Lax; HttpOnly; ${process.env.NODE_ENV === "production" ? "Secure;" : ""}`
                : "",
            `gmail_user_email=${profile.data.emailAddress}; Path=/; Max-Age=${60 * 60 * 24 * 7
