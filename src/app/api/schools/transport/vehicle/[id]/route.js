...
export const PUT = withSchoolAccess(async function PUT(req, props) {
  ...
  try {
    ...
    if (data.rcExpiry === "") data.rcExpiry = null;else
    if (data.rcExpiry) data.rcExpiry = new Date(data.rcExpiry);

    if (data.insuranceExpiry === "") data.insuranceExpiry = null;else
    if (data.insuranceExpiry) data.insuranceExpiry = new Date(data.insuranceExpiry);

    if (data.pucExpiry === "") data.pucExpiry = null;else
    if (data.pucExpiry) data.pucExpiry = new Date(data.pucExpiry);

    if (data.maintenanceDue === "") data.maintenanceDue = null;else
