import { withSchoolAccess } from "@/lib/api-auth"; // app/api/schools/[schoolId]/attendance/stats/route.js
    ...
    const result = await remember(cacheKey, async () => {
      ...
      if (userId) {
        ...
        const calculatedStats = recentAttendance.reduce((acc, record) => {
          const status = record.status;
          if (status === 'PRESENT') acc.totalPresent++;else
          if (status === 'ABSENT') acc.totalAbsent++;else
          if (status === 'LATE') acc.totalLate++;else
          if (status === 'HALF_DAY') acc.totalHalfDay++;else
          if (status === 'ON_LEAVE') acc.totalLeaves++;
          return acc;
        ...
        const yearlyAggregate = allYearAttendance.reduce((acc, record) => {
          if (record.status === 'PRESENT') acc.totalPresent++;else
          if (record.status === 'ABSENT') acc.totalAbsent++;else
          if (record.status === 'LATE') acc.totalLate++;else
          if (record.status === 'HALF_DAY') acc.totalHalfDay++;else
          if (record.status === 'ON_LEAVE') acc.totalLeaves++;
          return acc;
