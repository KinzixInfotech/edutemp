import { withSchoolAccess } from "@/lib/api-auth"; // app/api/schools/[schoolId]/inventory/items/[itemId]/route.js
...
export const PUT = withSchoolAccess(async function PUT(request, { params }) {
  try {
    const { schoolId, itemId } = await params;
...
export const DELETE = withSchoolAccess(async function DELETE(request, { params }) {
  try {
    const { schoolId, itemId } = await params;
    try {
