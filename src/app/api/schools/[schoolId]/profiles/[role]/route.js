...
const studentSchema = baseUserSchema.extend({
  ...
  profilePicture: z.string().optional().nullable(),
  gender: z.string().toUpperCase(),
  religion: z.enum(["HINDU", "MUSLIM", "CHRISTIAN", "SIKH", "BUDDHIST", "JAIN", "PARSI", "JEWISH", "OTHER", "PREFER_NOT_TO_SAY"]).optional().nullable(),
  address: z.string().optional().default(""),
  fatherName: z.string().optional(),
  motherName: z.string().optional(),
  fatherMobileNumber: z.string().optional(),
...
export const POST = withSchoolAccess(async function POST(req, context) {
  let createdUserId = null;
  ...
  try {
    ...
    if (parsed.contactNumber && parsed.contactNumber.length >= 10) {
      let phoneExists = false;
      if (mappedRole === "TEACHING_STAFF") {
        phoneExists = await prisma.teachingStaff.findFirst({ where: { schoolId, contactNumber: parsed.contactNumber } });
      } else if (mappedRole === "NON_TEACHING_STAFF") {
        phoneExists = await prisma.nonTeachingStaff.findFirst({ where: { schoolId, contactNumber: parsed.contactNumber } });
      } else if (mappedRole === "PARENT") {
        phoneExists = await prisma.parent.findFirst({ where: { schoolId, contactNumber: parsed.contactNumber } });
      }
      if (phoneExists) {
        return NextResponse.json({ error: "A profile with this contact number already exists in this school" }, { status: 409 });
      }
    }
    ...
    if (parsed.dob) {
      const age = differenceInYears(new Date(), new Date(parsed.dob));
      if (age < 3) return NextResponse.json({ error: "Date of birth results in age less than 3 years" }, { status: 400 });
      if (age > 100) return NextResponse.json({ error: "Date of birth results in age greater than 100 years" }, { status: 400 });
      if (new Date(parsed.dob) > new Date()) return NextResponse.json({ error: "Date of birth cannot be in the future" }, { status: 400 });
    }
    ...
    if (["STUDENT", "TEACHING_STAFF", "NON_TEACHING_STAFF"].includes(mappedRole)) {
      if (rawRole?.toLowerCase() !== "busdrivers") {
        activeAcademicYear = await prisma.academicYear.findFirst({ where: { schoolId, isActive: true } });
        if (!activeAcademicYear) {
          throw new Error("No active academic year found. Please create one before creating users.");
        }
      }
    ...
    const created = await prisma.$transaction(async (tx) => {
      // ✅ Renamed from `role` to `roleRecord` to avoid shadowing rawRole
        ...
        case "PARENT":
          profile = await tx.parent.create({
            data: {
              ...
              emergencyContactNumber: parsed.emergencyContactNumber || null,
              emergencyContactRelation: parsed.emergencyContactRelation || null,
              ...(parsed.linkedStudentIds?.length ? {
                studentLinks: {
                  create: parsed.linkedStudentIds.map((id) => ({
                    student: { connect: { userId: id } }
                  }))
    ...
    // ✅ FIXED: use rawRole (not `role` which was shadowed by the Prisma roleRecord inside the transaction)
    // Old code: const cachePrefix = roleCacheMap[role] || role  → role = Prisma object = undefined key
    // New code: switch on rawRole directly, match exact generateKey() prefixes
    try {
      switch (rawRole?.toLowerCase()) {
        ...
        case 'busdrivers':
          ...
          break;
        default:
          await invalidatePattern(`${rawRole}*`);
      }
    } catch (cacheErr) {
      console.warn('Cache invalidation warning:', cacheErr?.message);
    }
    ...
    if (error?.constructor?.name === "PrismaClientKnownRequestError") {
      if (error.code === "P2002") {
        const target = error.meta?.target;
        const field = Array.isArray(target) ? target.join(", ") : target || "field";
        return NextResponse.json({ error: "Duplicate entry", message: `A record with this ${field} already exists.` }, { status: 409 });
      }
      if (error.code === "P2003") {
        return NextResponse.json({ error: "Invalid reference", message: "One of the selected references (class, section, etc.) is invalid." }, { status: 400 });
      }
      if (error.code === "P2025") {
        return NextResponse.json({ error: "Not found", message: "A required related record was not found." }, { status: 404 });
      }
      return NextResponse.json({ error: "Database error", message: "A database error occurred. Please try again." }, { status: 500 });
    }

    if (error?.constructor?.name === "PrismaClientValidationError") {
      return NextResponse.json({ error: "Invalid data", message: "Some of the submitted data is invalid. Please review your form and try again." }, { status: 400 });
    }

    if (error?.constructor?.name === "PrismaClientInitializationError") {
      return NextResponse.json({ error: "Service unavailable", message: "Unable to connect to the database. Please try again later." }, { status: 503 });
    }

    return NextResponse.json({ error: "Failed to create profile", message: "An unexpected error occurred. Please try again." }, { status: 500 });
  }
});