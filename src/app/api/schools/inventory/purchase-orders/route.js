import { withSchoolAccess } from "@/lib/api-auth"; // pages/api/inventory/purchase-orders.js
import prisma from "@/lib/prisma";
import { NextResponse } from "next/server";

export const GET = withSchoolAccess(async function GET(request) {
  try {
...
export const POST = withSchoolAccess(async function POST(request) {
  try {
...
export const PUT = withSchoolAccess(async function PUT(request) {
  try {
    const body = await request.json();
...
export const DELETE = withSchoolAccess(async function DELETE(request) {
  try {
    const body = await request.json();
    const { id } = body;
