...
function defineStepper(...steps) {
    ...
    function StepperContainer({ children, className, ...props }) {
...
const classForNavigationList = function ({ variant }) {
    return "flex gap-2" + (variant === "horizontal" ? " flex-row items-center justify-between" : variant === "vertical" ? " flex-col" : variant === "circle" ? " flex-row items-center justify-between" : "");
};

const classForSeparator = function ({ orientation, labelOrientation }) {
    return [
        "bg-muted",
        "data-[state=completed]:bg-primary data-[disabled]:opacity-50",
        "transition-all duration-300 ease-in-out",
        orientation === "horizontal" ? "h-0.5 flex-1" : orientation === "vertical" ? "h-full w-0.5" : "",
